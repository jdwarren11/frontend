(function() {
  var BusinessLogic = function() {
    ...
    this.checkLandMine = function(userInput) {
      var diffArr = []
      for (var i = 0; i < this.landMineNumbers.length; i += 1) {
        diffArr.push(Math.abs(this.landMineNumbers[i] - userInput));
      }
      dist = Math.min.apply(Math, diffArr);
      console.log("distance from land mine number", dist)
      return dist 
    };

    // this is main number evaluator that checks the user input against the secret number
    // to see if it is greater less or equal to the result
    ...
    this.numberEvaluator = function(userInput) {
      ...
      var success = false;
      if (valid) {
        var land = this.checkLandMine(userInput);
        if (land === 0) {
          console.log('You landed on a Land Mine number! You lose!');
          ...
          console.log('You were ' + land + ' away from a land mine number.');
          console.log("Your number was valid. Thanks for playing!");
          success = "message1";
          return success;
        } else if (userInput < this.secretNumber) {
